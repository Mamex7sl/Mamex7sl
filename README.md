👋 Hello, I'm Mohammed Shifa 
A passionate Software Engineer | Full-Stack Developer 🚀  

---

### 💻 What I Build
I design and develop full-stack web applications that are:

🌟 **Fast**  
🎯 **Scalable**  
💡 **Problem-solving**

I’m passionate about turning ideas into real products and working with clients who value clean, functional design.  
If you’re looking for someone who can handle both frontend & backend, you’re in the right place.

👉 Check my portfolio here: [My Portfolio](https://mohammed-portfolio.com)  
📧 Email me: mohammedshifa800@gmail.com  

---

### 🛠️ My Tech Stack
**Frontend:** HTML, CSS, JavaScript, React, Next.js, TypeScript, Tailwind, Bootstrap  
**Backend:** Node.js, Express.js, PHP, Laravel  
**Database:** MySQL, PostgreSQL, MongoDB, Firebase  

---

### 🧱 Projects I’ve Built

| Project Name | Description | Tech Stack |
| ------------ | ----------- | ---------- |
| AgriConnect Ethiopia | Platform connecting farmers with buyers & market prices | React, Node.js, MongoDB |
| YouTube Clone | Video platform with custom API integration | React, Redux, Firebase |
| School Registration System | PHP-based student registration portal | PHP, MySQL |
| eCommerce App | Online shopping platform with cart & admin panel | React, Redux Toolkit, Express |
| Login System | Full auth system with JWT & React | Node.js, MongoDB |

---

 

---

### 📬 Contact Me

| Platform | Link |
| -------- | ---- |
| 🌐 Portfolio |[My Portfolio](https://mohammed-portfolio.com)|
| 📧 Email | mohammedshifa800@gmail.com |
| 💼 LinkedIn | {https://www.linkedin.com/in/mohammed-shifa-3019ba357/}
| 🧳 Upwork | [Upwork Profile](https://www.upwork.com/freelancers/~01f7cdeff5503ce72e?nav_dir=pop) |
| 💸 Fiverr | [Fiverr Profile](#) |
| 💬 Telegram | @Mamex7sl |
| 🐦 X (Twitter) | @username |

---

### 💡 What’s Next?
📚 Currently Learning: Next.js, Machine Learning, Mobile App Development  
🤝 Open to: Freelance Projects, Collaborations, Mentorship  
🌱 Goal: Build solutions that solve real problems in Ethiopia and beyond  

If you’ve made it this far, **thank you for visiting my profile** 🙏  
Let’s build something amazing together! 💼💻
